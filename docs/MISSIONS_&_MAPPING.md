...
## Phase 1: Mission Control & High-Level Logic

**Goal:** Transform robots from reactive agents to mission-driven systems with coordinated behavior.

**Estimated Time:** 8-12 hours

### Step 1.1: Create Mission Controller Header

**File:** `include/MissionController.h`
...
struct Mission {
    ...
    Mission() : type(MISSION_IDLE), missionId(0), patrolRadius(0.0f), 
                startTime(0), timeoutMs(0), isComplete(false) {}
};
...
struct TaskBid {
    ...
    TaskBid() : taskId(0), robotId(0), cost(9999.0f), timestamp(0) {}
};
...
public:
    MissionController() 
        : myRole(ROLE_NONE), myRobotId(0), nextMissionId(1), 
    ...
    bool isMissionActive() const { return currentMission.type != MISSION_IDLE; }
...
### Step 1.2: Implement Mission Controller

**File:** `src/MissionController.cpp`
...
void MissionController::setMission(const Mission& mission) {
...
void MissionController::completeMission() {
    ...
    currentMission.type = MISSION_IDLE;
}
...
### Step 1.3: Integrate with Main

**File:** `include/main.h`

Add this after the navigator declaration:
...
// In setup(), after SwarmCommunicator initialization:
void setup() {
    // ... existing setup code ...
    
    // Initialize MissionController with robot ID from SwarmCommunicator
    uint16_t robotId = (_myState.mac[4] << 8) | _myState.mac[5];
    missionController.setRobotId(robotId);
    
    Serial.println("🎯 Mission Controller initialized");
}

// In loop(), after navigation update:
void loop() {
    // ... existing loop code ...
...
Add these commands to `processCommand()`:
...
### Step 1.5: Web UI Updates

**File:** `data/index.html`

Add this CSS section (in `<style>` tags):
...
- [ ] MissionController compiles
- [ ] CLI commands work
- [ ] Web UI loads
- [ ] Waypoint navigation works
- [ ] Role assignment affects behavior
- [ ] Task auction (if multi-robot)
